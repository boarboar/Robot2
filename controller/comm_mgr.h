...
class CommManager {
  public:
    ...
    enum RetCodes { CM_RC_OK=0, CM_RC_NODATA=-1, CM_RC_FAIL_CRC=-2, CM_RC_FAIL_BADSYN=-10, CM_RC_FAIL_BADCMD=-11, CM_RC_FAIL_BADREG=-12, CM_RC_FAIL_TOOMANYVALS=-13, CM_RC_FAIL_TOOLESSVALS=-14 };
  protected:
    struct Alarm
    {
      uint16_t uAlarmID;      
    ...
    enum Regs {REG_None=0, REG_ID=1, REG_STATUS=2, REG_SENS=3, REG_ALL=4, 
      REG_MOTOR_POWER=5, REG_MOVE=6, REG_STEER=7, REG_MOVE_BEAR=8, REG_ALARM=9, REG_ENC=10, 
      REG_RESET=100, REG_N_SENS=101, REG_CFG=102};
    uint8_t CRC();
    int16_t ReadInt() ;        
